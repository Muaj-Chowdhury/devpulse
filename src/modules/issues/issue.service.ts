...
class IssueService {
  ...
  async getAllIssuesFromDB(
    filters: TGetIssuesQuery,
  ): Promise<TStitchedIssue[]> {
    ...
    SELECT id , name ,role FROM users WHERE id = ANY(${reporterIds})`) as Array<{
      id: number;
      name: string;
      role: string;
    }>;
  ...
  async getSingleIssueFromDB(id: number): Promise<TStitchedIssue | null> {
    ...
    const issue = IssueResult[0] as IIssue;
    ...
    return {
      ...
      reporter: reporterResult.length
        ? (reporterResult[0] as TStitchedIssue["reporter"])
        : {
            id: reporter_id,
            name: "Unknown user",
            role: "contributor",
...
async getIssueByIdFromDB(id: number): Promise<IIssue | null> {
    const result = await sql`
      SELECT id, title, description, type, status, reporter_id, created_at, updated_at
      FROM issues
      WHERE id = ${id}
    ...
    return result.length === 0 ? null : (result[0] as IIssue);
}

  async UpdateIssueInDB(
    id: Number,
  ...
  async deleteIssueFromDB(id: number): Promise<boolean> {
    const result = await sql`
